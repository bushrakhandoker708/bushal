...
export async function GET(
  ...
  try {
    ...
    const recommendedIds = (fbtData as FBTRow[]).map((row) => row.product_b_id)
    ...
    const productMap = new Map<string, ProductRow>(
      ((products as ProductRow[]) ?? []).map((p: ProductRow) => [p.id, p])
    )
    
    const enrichedRecommendations: EnrichedRecommendation[] = (fbtData as FBTRow[])
      .map((row: FBTRow) => {
