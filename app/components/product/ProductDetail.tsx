  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-bushal-ivoryDeep aspect-[4/5] group cursor-zoom-in">
        ...
        {/* Counter */}
        <div className="absolute bottom-4 right-4 bg-bushal-forest/80 backdrop-blur-md text-bushal-ivory text-xs font-semibold px-3 py-1.5 rounded-full">
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
      ...
      <div className="flex flex-col animate-fade-up pb-28 lg:pb-0">
        {/* Category / Breadcrumb */}
        <p className="text-xs font-semibold tracking-[0.2em] uppercase text-bushal-copper mb-3">
        ...
        <div className="flex items-baseline gap-4 mb-8">
          ...
          {discountedPrice && (
            <>
              <span className="text-xl text-bushal-inkSoft line-through">
                {formatPrice(product.price)}
              </span>
              <span className="text-xs font-bold tracking-wider uppercase text-bushal-success bg-bushal-successBg px-2.5 py-1 rounded-full">
        ...
        {/* Description */}
        {product.description && (
          <p className="text-bushal-inkMid leading-relaxed text-base mb-8">
            {product.description}
          </p>
        )}
        ...
        <div className="flex items-center gap-2 mb-6">
          {product.in_stock ? (
            <>
              ...
              <span className="text-sm font-semibold text-bushal-forest">In Stock & Ready to Ship</span>
            </>
          ) : (
            <>
              <span className="w-2 h-2 rounded-full bg-bushal-danger" />
              <span className="text-sm font-semibold text-bushal-danger">Currently Unavailable</span>
            </>
          )}
        </div>
        ...
        <div className="hidden lg:flex items-center gap-4 mb-8">
          <div className="flex items-center border border-bushal-border rounded-xl overflow-hidden bg-bushal-surface">
            ...
            <span className="w-12 h-12 flex items-center justify-center text-bushal-forest font-bold text-lg border-x border-bushal-border">
              {quantity}
            </span>
            <button
          ...
          <button
            ...
            className={cn(
              'flex-1 h-12 rounded-xl font-semibold text-sm tracking-wider uppercase transition-all duration-300 flex items-center justify-center gap-2',
              product.in_stock
                ? added
        ...
        <div className="grid grid-cols-3 gap-4 pt-8 border-t border-bushal-border">
          {[
            ...
            <div key={label} className="flex flex-col items-center text-center gap-1">
              ...
              <span className="text-xs font-bold text-bushal-forest uppercase tracking-wide">{label}</span>
              <span className="text-[11px] text-bushal-inkSoft">{sub}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-bushal-surface/95 backdrop-blur-xl border-t border-bushal-border p-4 safe-bottom shadow-[0_-10px_40px_rgba(27,58,45,0.1)]">
        <div className="flex items-center gap-3">
          <div className="flex items-center border border-bushal-border rounded-lg overflow-hidden bg-bushal-ivory">
            <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center text-bushal-forest text-lg">
              −
            </button>
            <span className="w-8 text-center font-bold text-bushal-forest">{quantity}</span>
            <button onClick={() => setQuantity((q) => q + 1)} className="w-10 h-10 flex items-center justify-center text-bushal-forest text-lg">
              +
            </button>
          </div>
          <button
            ...
            disabled={!product.in_stock}
            className={cn(
              'flex-1 h-12 rounded-lg font-bold text-sm tracking-wider uppercase transition-all duration-300 flex items-center justify-center gap-2',
              product.in_stock
                ? added
                  ? 'bg-bushal-success text-white'
