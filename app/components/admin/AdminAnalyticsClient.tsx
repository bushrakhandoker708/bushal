  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg className="transform -rotate-90 w-full h-full">
        <circle cx={size / 2} cy={size / 2} r={radius} stroke="currentColor" strokeWidth={strokeWidth} fill="transparent" className="text-bushal-ivoryDeep" />
        <circle cx={size / 2} cy={size / 2} r={radius} stroke="currentColor" strokeWidth={strokeWidth} fill="transparent" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" className={cn(color, "transition-all duration-1000 ease-out")} />
      </svg>
  ...
  return (
    <div className={cn('bg-gradient-to-br rounded-2xl border p-5 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 animate-fade-in-up', styles[accent] || styles.forest)} style={{ animationDelay: `${delay}ms` }}>
      <div className="flex items-start justify-between mb-3">
        <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center shadow-sm', iconBgStyles[accent] || iconBgStyles.forest)}>
          {icon}
        </div>
        {trend && (
      ...
      <div>
        <p className={cn('text-2xl font-extrabold tracking-tight', textStyles[accent] || textStyles.forest)}>
    ...
    <div className="bg-bushal-surface rounded-2xl border border-bushal-border p-6 animate-fade-in-up">
      ...
      <div className="relative h-48">
        <svg viewBox="0 0 100 100" className="w-full h-full" preserveAspectRatio="none">
          ...
          <polyline points={points} fill="none" stroke="#B87333" strokeWidth="0.5" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-sm" />
          {data.map((_, i) => {
            const x = (i / (data.length - 1)) * 100
            const y = 100 - (data[i].revenue / maxRevenue) * 100
            return (
              <circle key={i} cx={x} cy={y} r={hoveredIndex === i ? "1.5" : "1"} fill="#B87333" className="transition-all duration-200 cursor-pointer" onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)} />
            )
          })}
        </svg>
        {hoveredIndex !== null && (
          <div className="absolute bg-bushal-forest text-white text-xs px-2 py-1 rounded-lg shadow-lg pointer-events-none animate-scale-in" style={{ left: `${(hoveredIndex / (data.length - 1)) * 100}%`, top: `${100 - (data[hoveredIndex].revenue / maxRevenue) * 100 - 15}%`, transform: 'translateX(-50%)' }}>
      ...
      <div className="flex justify-between mt-2">
        {data.map((d, i) => (
          <div key={i} className="text-[10px] text-bushal-inkSoft text-center flex-1">{d.date.split(' ')[0]}</div>
        ))}
      </div>
    </div>
  )
        ...
        <div className="relative" style={{ width: size, height: size }}>
          <svg viewBox="0 0 100 100" className="transform -rotate-90 w-full h-full">
            {data.map((segment, i) => {
              ...
              return (
                <circle key={i} cx="50" cy="50" r="40" fill="transparent" stroke={segment.color} strokeWidth={hoveredIndex === i ? "12" : "10"} strokeDasharray={strokeDasharray} strokeDashoffset={strokeDashoffset} className="transition-all duration-300 cursor-pointer" onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)} />
              )
            })}
          </svg>
          <div className="absolute inset-0 flex items-center justify-center">
        ...
        <div className="flex-1 space-y-2">
          {data.map((d, i) => (
            <div key={i} className={cn('flex items-center justify-between p-2 rounded-lg transition-all duration-200', hoveredIndex === i ? 'bg-bushal-ivoryDeep scale-105' : '')} onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
              ...
              <span className="text-xs font-bold text-bushal-ink">{formatPrice(d.value)}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        {activities.map((activity, i) => (
          <div key={activity.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-bushal-ivoryDeep transition-all duration-200 group animate-fade-in" style={{ animationDelay: `${i * 50}ms` }}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-sm font-semibold text-bushal-forest truncate">Order #{activity.id.slice(0, 8).toUpperCase()}</p>
                <span className={cn('text-[10px] font-bold px-1.5 py-0.5 rounded-full', getStatusColor(activity.status))}>{activity.status}</span>
              </div>
              <p className="text-[11px] text-bushal-inkSoft">{new Date(activity.created_at).toLocaleDateString('en-BD', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
            </div>
...
function StockAlert({ summary }: { summary: Summary }) {
  ...
  return (
    <div className="space-y-3 animate-fade-in-up">
      {alerts.map((alert, i) => (
        <div key={i} className={cn('rounded-2xl border p-4 flex items-center justify-between', alert.bgColor, alert.borderColor)} style={{ animationDelay: `${i * 100}ms` }}>
          <div className="flex items-center gap-3">
          ...
          <div className={cn('w-10 h-10 rounded-full flex items-center justify-center text-white font-bold', alert.color)}>{alert.count}</div>
        </div>
      ))}
    </div>
  )
...
export default function AdminAnalyticsClient({ summary, dailyRevenue, monthlyData, topProducts, categoryPerformance, recentActivity, products, expenses }: Props) {
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex items-center justify-between animate-fade-in-up">
        ...
        <button onClick={() => setShowExpenseModal(true)} className="inline-flex items-center gap-2 bg-bushal-copper text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-bushal-copperLight transition-all shadow-lg shadow-bushal-copper/25 hover:-translate-y-0.5 active:scale-[0.98]">
        ...
        {expenses.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <div className="w-16 h-16 bg-bushal-ivoryDeep rounded-full flex items-center justify-center mx-auto mb-3">
              <svg className="w-7 h-7 text-bushal-inkSoft" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
            </div>
